...
function prepareContext(context) {
  ...
  let mappedArgs = Object.entries(targetedContext.args).reduce(
    ...
    includedArgs = Object.entries(mappedArgs).reduce((acc, [key2, val]) => {
      ...
      return (
        P(argType, mappedArgs, targetedContext.globals) && (acc[key2] = val),
        acc
      );
    }, {});
...
var globalProjectAnnotations = {},
  DEFAULT_STORY_TITLE = 'ComposedStory',
...
function composeStory(
  ...
  componentAnnotations.title =
    componentAnnotations.title ?? DEFAULT_STORY_TITLE;
  ...
  return Object.assign(
      ...
      if (
        ...
        story.title !== DEFAULT_STORY_TITLE &&
          ...
          storyName === DEFAULT_STORY_NAME &&
            Object.keys(context.args).length > 0 &&
            (humanReadableIdentifier = `${humanReadableIdentifier} (${Object.keys(context.args).join(', ')})`),
          console.warn(esm_default`Some stories were not cleaned up before rendering '${humanReadableIdentifier}'.
    ...
    },
    {
      ...
      storyName,
      load: async () => {
        for (let { callback } of [...cleanups].reverse()) await callback();
        (cleanups.length = 0), (previousCleanupsDone = !0);
    ...
    mount: async ({ mount, page }, use) => {
      await use(async (storyRef, ...restArgs) => {
        ...
        await page.evaluate(async (wrappedStoryRef) => {
          let unwrappedStoryRef =
            await globalThis.__pwUnwrapObject?.(wrappedStoryRef);
          return (
            '__pw_type' in unwrappedStoryRef
              ? unwrappedStoryRef.type
              : unwrappedStoryRef
          )?.load?.();
        }, storyRef);
...
var CSF_CACHE_SIZE = 1e3,
  ...
  StoryStore = class {
    ...
    async cleanupStory(story) {
      ...
      if (callbacks)
        for (let callback of [...callbacks].reverse()) await callback();
...
var getQueryString = ({ selection, extraParams }) => {
  ...
  setPath = (selection) => {
    ...
    let query = getQueryString({ selection }),
      { hash = '' } = document22.location;
    (document22.title = selection.storyId),
  ...
  getSelectionSpecifierFromPath = () => {
    if (typeof document22 < 'u') {
      let query = import_qs.default.parse(document22.location.search, {
          ignoreQueryPrefix: !0,
        }),
        args2 =
...
`,
        ...
        (document3.getElementById('error-stack').innerHTML =
